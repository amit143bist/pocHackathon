...
public class ServerReceiver implements VerificationCodeReceiver {
	 ...
	 */
	final Semaphore waitUnlessSignaled = new Semaphore(
			0 /* initially zero permit */);
	...
	/**
	 * Constructor that starts the server on {@link #LOCALHOST} and an unused
	 * port.
	 *
	 * <p>
	 ...
	 */
	ServerReceiver(String host, int port, String successLandingPageUrl, String failureLandingPageUrl) {
	 ...
	 *            Port to use or {@code -1} to select an unused port
	 */
	ServerReceiver(String host, int port, String callbackPath, String successLandingPageUrl,
	...
	public String getRedirectUri() throws IOException {
		/*
		 * server = new Server(port != -1 ? port : 0); Connector connector =
		 * server.getConnectors()[0]; connector.setHost(host);
		 * server.addHandler(new CallbackHandler()); try { server.start(); port
		 * = connector.getLocalPort(); } catch (Exception e) {
		 * Throwables.propagateIfPossible(e); throw new IOException(e); }
		 */
	...
	public static final class Builder {
		...
		public ServerReceiver build() {
			return new ServerReceiver(host, port, callbackPath, successLandingPageUrl, failureLandingPageUrl);
		}
